...
select re.id, (YEAR(re.start_of_week) * 10000 + MONTH(re.start_of_week) * 100 + DAY(re.start_of_week)) as startDate, CONVERT(VARCHAR(11),re.start_of_week,106) as ss,
(YEAR(re.end_of_week) * 10000 + MONTH(re.end_of_week) * 100 + DAY(re.end_of_week)) as endDate, CONVERT(VARCHAR(11),re.end_of_week,106) as ae,
re.action, re.ct
from(
