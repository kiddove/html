...
(function( _V_ ) {
	...
	_V_.plugin( 'resolutionSelector', function( options ) {
	    ...
	    // modified by paul
	    // option string like "OBDE"
		player.setCurrentRes = function (res) {
		...
		for (var ii = 0; ii < player.pageObj.multstream.length; ii++) {
		    ...
		    switch (s) {
		        ...
		        case "b":
		            {
		                src["data-res"] = "240";
		                src.src = str.replace(/_([bBdDeE])./g, "_B.");
		                if (src.src == str) {
		                    player.setCurrentRes(src["data-res"]);
		                    current_res = src['data-res'];
		                }
		                break;
		            }
		        case "D":
		        case "d":
		            {
		                src["data-res"] = "480";
		                src.src = str.replace(/_([bBdDeE])./g, "_D.");
		                if (src.src == str) {
		                    player.setCurrentRes(src["data-res"]);
		                    current_res = src['data-res'];
		                }
		        ...
		        case "e":
		            {
		                src["data-res"] = "720";
		                src.src = str.replace(/_([bBdDeE])./g, "_E.");
		                if (src.src == str) {
		                    player.setCurrentRes(src["data-res"]);
		                    current_res = src['data-res'];
		                }
		    ...
		    if (src["data-res"].length > 0) {
		        if (typeof available_res[src['data-res']] !== 'object') {

		            available_res[src['data-res']] = [];
		            available_res.length++;
		        }

		        available_res[src['data-res']].push(src);
		    }
		}
		...
		player.changeRes = function( target_resolution ) {	var video_el = player.el().firstChild,
